...
function BuildCities()
  ...
  global.coe.teams = settings.global["coe_teams"].value
  global.coe.create = settings.global["coe_create"].value
  local city = nil

  for _, slot in pairs(indexes) do
    city = ReadCityFromSettings(slot)
...
function ReadCityFromSettings(slot)
...
function IsValidSpawnSettings()
  local result = false
  local fe_spawn_x = settings.global["spawn-x"].value
  local fe_spawn_y = settings.global["spawn-y"].value
  local coe_spawn_x = settings.global["coe_spawn-x"].value
  local coe_spawn_y = settings.global["coe_spawn-y"].value
